...
`min` _(string)_ — The earliest time that can be selected. The value should be in 24-hour format (HH:MM).

`max` _(string)_ — The latest time that can be selected. The value should be in 24-hour format (HH:MM).
...
if ( ! empty( $opening_time ) ) {
	// Format the time according to site settings.
...
if ( ! empty( $business_hours ) ) {
	...
	foreach ( $business_hours as $day => $hours ) {
		echo '<tr>';
		echo '<td>' . esc_html( ucfirst( $day ) ) . '</td>';
		echo '<td>' . esc_html( $hours['open'] ) . ' - ' . esc_html( $hours['close'] ) . '</td>';
		echo '</tr>';
	}
...
'opening_time' => array(
	...
	'default'     => '09:00',
	'conditions'  => array(
		array( 'field' => 'has_opening_hours', 'value' => true ),
	),
),
```
...
- The `step` attribute can be used to limit time selection to specific intervals (value in seconds, e.g., `900` for 15-minute intervals)
